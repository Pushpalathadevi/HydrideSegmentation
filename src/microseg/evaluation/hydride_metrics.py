...
def _component_orientations_and_sizes(mask: np.ndarray) -> tuple[list[float], list[int], np.ndarray]:
    labels = measure.label(mask > 0)
...
def scientific_distance_metrics(gt_mask: np.ndarray, pred_mask: np.ndarray) -> dict[str, float]:
    """Compute scientific-distribution distances for binary masks."""

    gt_orient, gt_sizes, _ = _component_orientations_and_sizes(gt_mask)
    pr_orient, pr_sizes, _ = _component_orientations_and_sizes(pred_mask)
