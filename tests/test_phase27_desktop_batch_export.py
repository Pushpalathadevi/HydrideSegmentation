...
def test_phase27_batch_export_outputs(tmp_path: Path) -> None:
    ...
    assert all(str(row.get("input_preview_path", "")).startswith("runs/") for row in payload["rows"])
    assert all(str(row.get("mask_preview_path", "")).startswith("runs/") for row in payload["rows"])
    assert all(str(row.get("overlay_preview_path", "")).startswith("runs/") for row in payload["rows"])
    assert all(str(row.get("run_summary_path", "")).startswith("runs/") for row in payload["rows"])
