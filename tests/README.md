...
- `test_phase36_in_memory_web_jobs.py` exact 10 MB default, content/extension matching, upload preview and sample-thumbnail contracts, memory-only provenance, asynchronous progress events, successful terminal reports, and rejection before queueing
- `test_web_downloads_and_reports.py` metadata-driven repository-bound download catalog, version/header navigation, top-positioned Run action, split input/mask viewer and synchronized magnifier contract, scientific two-page PDF, XLSX/PNG/JSON ZIP bundle, and completed-job report endpoints
